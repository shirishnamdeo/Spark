[https://github.com/BenFradet/spark-kaggle/tree/master/titanic/src/main]
[https://github.com/BenFradet/spark-ml-examples]
...
object Titanic {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // The coalesce method reduces the number of partitions in a DataFrame. coalesce(numOfPartitions)
    // But coalesce cannot increase the number of partitions, (atleast cannot re-distribute the data from exsiting partition)
    // use repartition instead
    // But repartition does not attempt to minimize data movement like the coalesce algorithm.
    ...
    def createExtraFeatures(trainDF: DataFrame, testDF: DataFrame): (DataFrame, DataFrame) = {
        ...
        val title: ((String, String) => String) = {
            case (Pattern(t), sex) => titles.get(t) match {
                    case Some(tt) => tt
                    case None     => if (sex == "male") "Mr" else "Mrs"
            }
            case _ => "Mr"
        }
    ...
    def loadData(trainFile: String, testFile: String, spark: SparkSession): (DataFrame, DataFrame) = {
        val nullable = true
        ...
        val trainDF = spark.read.format("csv").option("header", "true").schema(trainSchema).load(trainFile)
        val testDF = spark.read.format("csv").option("header", "true").schema(testSchema).load(testFile)
